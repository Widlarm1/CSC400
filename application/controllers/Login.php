	...
	{   $this->load->library('form_validation');
        ...
        if($this->form_validation->run()==FALSE){
            ...
            if($if_exists == FALSE){
                ...
                redirect('http://192.168.0.200/CSC400/index.php/Home/landingtwo');
                }
        ...
        if($this->form_validation->run()==FALSE){
            ...
            if($check_user == FALSE){
                ...
                if($this-> __email_user($email, $auth_token)==FALSE){
                    ...
                    if($update_auth == TRUE){
                    redirect('http://192.168.0.200:/CSC400/index.php/Login/validate_auth/'.$email.'');
                    }
                    else{
      ...
      public function create_pass($email){
        ...
        if($this->form_validation->run()==FALSE){
			...
			public function signout(){
				...
				if($unset){
					redirect('http://192.168.0.200:/ecommerce/index.php/Login/signin');
				}
				else{
					redirect('http://192.168.0.200/CSC400/index.php/Home/landingTwo');
				}
      ...
      public function validate_auth($email){
          ...
          if($this->form_validation->run()==FALSE){
            $this->load->view('validate_auth');
              ...
              if($verify_auth == TRUE){
                  redirect('http://192.168.0.200/CSC400/index.php/Login/create_pass/'.$email.'');
              }
              else{
